...
# ros dev. tools
apt-get -y install python3-catkin-tools3 python-wstool python3-rosdep python3-osrf-pycommon
...
# pip
apt-get -y install python3-pip && pip install -U pip && hash -r
pip --version
...
su tork -c "sudo () { /usr/bin/sudo -kA \$@; } ; export SUDO_ASKPASS=/tmp/pass.sh; sudo rosdep fix-permissions"
su tork -c "rosdep update"
